...
import kr.djspi.pipe01.databinding.ActivityNavermapBinding
...
class NaverMapActivity : LocationUpdate(), OnMapReadyCallback, Serializable {
    ...
    private lateinit var mapBinding: ActivityNavermapBinding
    ...
    private inner class SetBottomSheet {
        ...
        init {
            ...
            behavior = from(mapBinding.nmapBottomSheet)
            setContentView()
        }

        private fun setContentView() {
            mapBinding.nmapBottomSheetText.setOnClickListener {
                when (behavior.state) {
                    STATE_EXPANDED -> {
                        behavior.state = STATE_COLLAPSED
                    }
            ...
            val bottomSheetHeight = mapBinding.nmapBottomSheet.height
            behavior.addBottomSheetCallback(object : BottomSheetCallback() {
                override fun onStateChanged(bottomSheet: View, newState: Int) {
                    beginDelayedTransition(mapBinding.nmapBottomSheet, transition)
                    when (newState) {
                        STATE_EXPANDED -> {
