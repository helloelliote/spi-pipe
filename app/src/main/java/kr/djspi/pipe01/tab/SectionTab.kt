...
import kotlinx.android.synthetic.main.tab_plane.text_vertical
...
class SectionTab : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        json["file_section"].asString?.let {
            ...
            planeImageView.setImageResource(resources.getIdentifier(resId, "drawable", BaseActivity.defPackage))
        }
        text_vertical.text = json["vertical"].asString
        text_depth.text = json["depth"].asString
        text_spec.text = "${json["header"].asString} ${json["spec"].asString} ${json["unit"].asString}"
    ...
    private fun setTranslation(noV: Boolean, dX: Float = 0.0f) {
        if (noV) text_vertical.visibility = View.GONE
        text_depth.translationX = dX * screenRatio
        text_depth.translationY = 77.5f * screenRatio
        text_vertical.translationY = -475.0f * screenRatio
